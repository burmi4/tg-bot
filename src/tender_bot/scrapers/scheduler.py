...
from tender_bot.filters import is_tender_allowed
...
def _format_tender_message(tender: TenderItem) -> str:
    ...
    if tender.max_cost and tender.max_cost != "Не указана":
        lines.append(f"💰 Предельная стоимость: {tender.max_cost}")
    elif tender.max_cost == "Не указана":
        lines.append("💰 Предельная стоимость: Не указана")
    if tender.publish_period:
        lines.append(f"📆 Сроки размещения: {tender.publish_period}")
    if tender.submission_deadline:
        lines.append(f"⏳ Подача заявок до: {tender.submission_deadline}")
    if tender.execution_period:
        lines.append(f"🛠 Срок производства работ: {tender.execution_period}")
    if tender.status:
...
async def _poll_tenders(bot: Bot) -> None:
    ...
    # Filter out already seen tenders and ignored ones, then notify subscribers
    new_tenders: list[TenderItem] = []
    for tender in all_tenders:
        # Ignore tenders that match the blocklist
        if not is_tender_allowed(tender, settings.stop_words):
            continue
